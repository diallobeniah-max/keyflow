...
| Range | Width | Intent |
|---|---:|---|
| Narrow defensive mode | below 760px | Prevent clipping in unusually narrow windows; not the primary experience |
...
* Electron minimum window width is `--layout-min-window-width` (760px); minimum height is `--layout-min-window-height` (640px).
...
* Below 760px, the same drawer behaviour remains defensive; do not create a phone-specific navigation model.
...
* Narrow defensive mode uses 16px padding.
