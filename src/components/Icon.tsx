...
import {
  AppWindow, ArrowsOutCardinal, ArrowClockwise, ArrowUUpLeft, ArrowUUpRight,
  BatteryFull, Bell, BookmarkSimple, Books, Briefcase, Browser, Bug, Calendar, Camera,
  CaretDown, CaretLeft, CaretRight, CaretUp, Check, CheckCircle, Clipboard,
  Clock, ClockCounterClockwise, Code, Coffee, Command, Compass, Copy, Cpu,
  Database, Desktop, Drop, Export, File, FilmStrip, Flag, Flame, Folder,
  GearSix, GitBranch, Globe, Hash, Headphones, Heart, Highlighter, Key,
  Keyboard, Lightning, List, Lock, MagnifyingGlass, MapPin, Microphone, Moon, Mouse,
  MusicNote, NotePencil, Pause, PencilSimple, Play, Plus, Power, Pulse, PushPin, ShieldCheck,
  SlidersHorizontal, Smiley, Sparkle, SpeakerHigh, SpeakerSlash, SquaresFour, Stack, Star, Sun,
  Table, Tag, Terminal, TextAlignCenter, TextAlignLeft, TextAlignRight, TextT, Trash,
  Tray, UploadSimple, UsersThree, VideoCamera, WifiHigh, Wrench, X,
...
export function Icon({ name, size = 20, className, strokeWidth = 1.8, style }: { name: IconName; size?: number; className?: string; strokeWidth?: number; style?: React.CSSProperties }) {
  ...
  if (PhosphorIcon) {
    return <PhosphorIcon size={size} weight={strokeWidth >= 2 ? "bold" : "regular"} className={className} style={style} aria-hidden="true" />;
  }
